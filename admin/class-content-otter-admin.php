  ...
  /**
	...
	*/
	public function admin_page_template(){
        ...
        include(plugin_dir_path( __FILE__ ) . 'partials/content-otter-admin-display.php');
	}
	...
	*/
	private function get_article($url){
	...
	*/
	 public function otter_save_post() {
		 ...
		 //add image classes
			...
			//remove doctype
			$content = preg_replace('/^<!DOCTYPE.+?>/', '', str_replace( array('<html>', '</html>', '<body>', '</body>'), array('', '', '', ''), html_entity_decode($doc->saveHTML())));
    ...
    $post = get_post($post_id);
		...
		echo json_encode(array('post'=>$post, 'admin_url'=>$post_admin_edit_url));
